...
fn rec_value(n: usize, m: usize, parant: usize, now_turn_is_first: bool) -> bool {
    let mut children = Vec::new();
    for i in 0..m {
        // println!("{}, {}", parant, i);
        if parant + i + 1 == n {
            // println!("{} {} {}", parant, parant + i+1, !turn_is_first);
            children.push(now_turn_is_first);
            break;
        }
        let buf =rec_value(n, m, parant + i + 1, !now_turn_is_first) ;
        // println!("{} {} {}", parant, parant + i+1, buf);
        children.push(buf);
    }
    if parant == 0 {
        second_judge(children)
    } else if now_turn_is_first {
