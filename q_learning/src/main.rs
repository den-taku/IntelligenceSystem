...
fn main() {
    ...
    {
        for _ in 0..100 {
            q1.borrow_mut().q_search_goal(start1, goal11, action_determiner1.clone(), next_state_determiner1.clone(), &mut learning_rate11, &mut times1);
            epsiron1 -= 0.0065;
            action_determiner1.borrow_mut().update_epsiron(epsiron1);
        }
    }
    {
        q1.borrow_mut().q_search_goal_print(start1, goal11, action_determiner1.clone(), next_state_determiner1.clone(), &mut learning_rate11, &mut times1);
    }
    println!("{}", q1.borrow());
    ...
    {
        for _ in 0..100 {
            q2.borrow_mut().q_search_goal2(start2, goal12, goal22, action_determiner2.clone(), next_state_determiner2.clone(), &mut learning_rate12, &mut times2);
            epsiron2 -= 0.0065;
            action_determiner2.borrow_mut().update_epsiron(epsiron2);
        }
    }
    {
        q2.borrow_mut().q_search_goal_print2(start2, goal12, goal22, action_determiner2.clone(), next_state_determiner2.clone(), &mut learning_rate12, &mut times2);
    }
    println!("{}", q2.borrow());
